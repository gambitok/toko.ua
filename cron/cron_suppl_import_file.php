...
$r = $db->query("SELECT `ID`, `SUPPL_ID`, `FILENAME`, `STATUS_COUNTER` FROM `IMPORT_SUPPLIER_FILES` WHERE `STATUS` = 1 AND `SUPPL_ID` = $suppl_id LIMIT 1;");
$n = $db->num_rows($r);

if ($n > 0) {

    $id         = (int)$db->result($r, 0, "ID");
    $suppl_id   = (int)$db->result($r, 0, "SUPPL_ID");
    $fileName   = $db->result($r, 0, "FILENAME");
    $counter    = (int)$db->result($r, 0, "STATUS_COUNTER");
