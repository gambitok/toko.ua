...
class CatalogExistClass extends CatalogueClass
{
    ...
    public function getPartsPaginationForm($n, $page, $sort = 0)
    {
        ...
        $list = str_replace(array("{pagination_range}", "{pred_disabled_class}", "{next_disabled_class}", "{link_pred}", "{link_next}"), array($pagination, ($page === 1) ? "disabled" : "", ($page === $pages_count) ? "disabled" : "", ($pred_page > 1) ? $prefix . "page=$pred_page" : ".", ($next_page > 1) ? $prefix . "page=$next_page" : "."), $list);
    ...
    public function checRedirects($filters): array
    {
        $status = 0;
        $arr = [
            "neolux%D0%92%C2%AE",
    ...
    public function showPartsCatalogueError($group_id, $mfa_id, $model, $status_auto, $status_auto_type, $typ_id, $filters_h1)
    {
        ...
        $form = str_replace(array("{h1_text}", "{vin_text}", "{parts_cars}"), array("<b>$filters_h1</b>", "<a class=\"blue-a\" onclick=\"$('#VinFormPhone').modal('show');\">{vin_order}</a>", $this->drawLoader()), $form);
    ...
    public function showPartsCatalogueParams($group_id, $page = 1, $filters = [], $params = [], $mfa_id = 0, $model = "", $model_id = 0, $status_auto = 0, $status_auto_type = 0, $source_link = "", $sort = 0): array
    {
        ...
        if (empty($art_id_str)) {
            ...
            $form = str_replace(array("{mfa_link}", "{model_link}", "{model_id_link}"), array($this->getManufactureLink($mfa_id), $this->getModelLink($model), $this->getModelIDLink($model_id)), $form);
        } else {
            ...
            $form = str_replace("{model_id_link}", $this->getModelIDLink($model_id), $form);
            $form = str_replace("{parts_name}", $group_text, $form);
            $form = str_replace("{parts_list}", $list, $form);
            $form = str_replace("{parts_h1}", $parts_h1, $form);
            $form = str_replace("{parts_count}", $parts_count, $form);
            ...
            $filters_style      = ($filters_count === 0) ? "none" : "";
            ...
            $form = str_replace("{filters_style}", $filters_style, $form);
     ...
     * */
    public function getPartsFiltersItems($group_id, $page = 1, $params = [], $mfa_id = 0, $model = "", $model_id = 0): array
    {
        $filters_btn = "";
        $count_values = 0;
        if (!empty($params)) {
    ...
    public function getPartsCatalogueParamsCars($group_id, $params, $status_auto = 0, $status_auto_type = 0, $typ_id = 0, $mfa_id = 0, $model = "", $model_id = 0)
    {
        ...
        if ($status_auto === 1) {
            ...
            if ($typ_id !== "") {
                ...
                $form = str_replace(array("{typ_text}", "{on_car_checked}", "{on_car_count}", "{on_all_checked}", "{on_all_count}"), array($typ_text, $car_checked, $car_count, $all_checked, $all_count), $form);
            }
    ...
    public function getPartsCatalogueSeo($group_id, $page = 1, $params = [], $source_link = "", $h1_text = "", $mfa_id = 0, $model = "", $model_id = 0, $status_auto = 0, $status_auto_type = 0, $typ_id = 0)
    {
        ...
        $form = str_replace(array("{seo_filters}", "{seo_auto}", "{seo_popular}", "{seo_style}", "{seo_filters_style}", "{seo_generate}", "{seo_generate_style}"), array("", "", "", "none", "none", "", "none"), $form);
    ...
    public function getGroupCarMfaList($group_id, $mfa_id_sel = 0, $status = 0)
    {
        ...
        $form = $this->getHtmlForm("catalog_exist/seo_content_auto");
        $form = str_replace(array("{seo_auto_title}", "{seo_auto_list}"), array($title, $list), $form);
