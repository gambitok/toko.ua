...
class PatternClass extends CatalogueClass
{
    ...
    public function initTemplateTable($template_id)
    {
        ...
        foreach ($products as $art_id => $params) {
            ...
            if ($stock > 0) {
                $r = $db->query("SELECT * FROM `XX_TABLE_TEMPLATE_$template_id` WHERE `art_id`='$art_id' LIMIT 1;");
                $n = $db->num_rows($r);
                if ($n == 0) {
    ...
    public function showProductsForm($template_id, $page = 1, $link = "")
    {
        ...
        if ($count_arts < $this->products_on_page) {
            $pages_count = 1;
        }
        ...
        foreach ($active_filters as $param_id => $values) {
            foreach ($values as $value_id) {
                if ($param_id == 0) {
                    ...
                    $value_name = $this->getCatalogueValueName($value_id, $template_id);
                }
                $h1 .= " $value_name";
            }
        }
        ...
        foreach ($arr as $param_id => $values) {
            ...
            foreach ($values as $value) {
                ...
                if ($param_id == 0) {
                    ...
                    $value_name = $this->getCatalogueValueName($value, $template_id);
                }
