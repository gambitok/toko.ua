...
class ParametersClass
{
    ...
    public function getCurrentProducts($template_id)
    {
        ...
        FROM `T2_CATALOGUES_ARTS` t2c 
            LEFT JOIN `T2_ARTICLES` t2a ON t2a.ART_ID=t2c.ART_ID
        WHERE t2c.`TEMPLATE_ID`='$template_id' GROUP BY t2c.`ART_ID`;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $art_id = $db->result($r, $i - 1, "ART_ID");
        ...
        $r = $db->query("SELECT * FROM `T2_CATALOGUES_ARTS` WHERE `TEMPLATE_ID`='$template_id';");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $art_id = $db->result($r, $i - 1, "ART_ID");
    ...
    public function getActiveProducts($current_products, $active_filters)
    {
        $active_products = [];
    ...
    public function showFiltersForm($template_id, $active_filters = [])
    {
        ...
        WHERE t2c.`TEMPLATE_ID`='$template_id' $where_arts 
        GROUP BY t2c.`ART_ID`, t2c.`VALUE_ID`;");
        $n = $db->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            $param_id = $db->result($r, $i - 1, "PARAM_ID");
            $value_id = $db->result($r, $i - 1, "VALUE_ID");
    ...
    public function getActiveProductsCount($template_id, $active_products)
    {
        $db = DbSingleton::getTokoDb();
        $where_arts = "";
        if (!empty($active_products)) {
