...
class CatalogueClass
{
    ...
    public function getArts(): string
    {
        ...
        $arts = [];
        $r = $dbt->query("SELECT `GROUP_ID` FROM `T2_TREE_GROUP_EXIST` WHERE 1;");
        $n = $dbt->num_rows($r);
        for ($i = 1; $i <= $n; $i++) {
            ...
            $r_check = $dbc->query("SELECT 1
            FROM INFORMATION_SCHEMA.TABLES
            WHERE TABLE_SCHEMA = 'toko_dba_cache' 
            AND TABLE_NAME = '$table'; ");
            $n_check = $dbc->num_rows($r_check);

            if ($n_check > 0) {
                $r2 = $dbc->query("SELECT `art_id` FROM `$table` WHERE `price` > 0");
                $n2 = $dbc->num_rows($r2);
                for ($j = 1; $j <= $n2; $j++) {
                    $art_id = $dbc->result($r2, $j - 1, "art_id");
                    $arts[] = $art_id;
                }
            }
        }
        ...
        return "SELECT 
            t2as.ART_ID, 
            t2a.ARTICLE_NR_DISPL, 
            t2b.BRAND_NAME, 
            IFNULL(t2n.NAME,'') as NAME, 
            t2n.INFO, 
            t2br.BARCODE 
        FROM 
            `T2_ARTICLES_STRORAGE` t2as
        LEFT OUTER JOIN 
            `T2_ARTICLES` t2a ON (t2a.ART_ID = t2as.ART_ID)
        LEFT OUTER JOIN 
            `T2_BRANDS` t2b ON (t2b.BRAND_ID = t2a.BRAND_ID)
        LEFT OUTER JOIN 
            `T2_NAMES` t2n ON (t2n.ART_ID = t2a.ART_ID)
        LEFT OUTER JOIN 
            `T2_BARCODES` t2br ON (t2br.ART_ID = t2a.ART_ID)
        WHERE 
            t2as.AMOUNT != 0 AND (IF (t2n.LANG_ID IS NOT NULL, t2n.LANG_ID = 16, TRUE)) AND t2a.ART_ID IN ($str)
        GROUP BY 
            t2a.ARTICLE_NR_DISPL;";
    }
    ...
    public function getCatalogCron(): array
    {
        ...
        $cur = $client->getClientCurrency($client_id);
        $curCap = $exRate->getExRateCaption($cur);
        ...
        for ($i = 1; $i <= $n; $i++) {
            $artId = $db->result($result, $i - 1, "ART_ID");
            $brandId = $db->result($result, $i - 1, "BRAND_ID");
            $artNrDs = $db->result($result, $i - 1, "ARTICLE_NR_DISPL");
            $brandName = $db->result($result, $i - 1, "BRAND_NAME");
            $artName = $db->result($result, $i - 1, "NAME");
            $info = $db->result($result, $i - 1, "INFO");

            $info = trim($info, " \n\r");
            $info = str_replace(array("\n", "\r"), "", $info);

            $price = $this->getArticlePriceClient($db, $artId, $client_id, $cur);
            $price = str_replace(".", ",", $price);

            $gtinStatus = ($artNrDs !== "") ? "yes" : "no";
            $gtin = $artNrDs;
            $mpn = $artNrDs;

            $formObj = new FormClass();
            $mainArticlePhoto = $formObj->getArticlePhoto($artId);
            $image = ($mainArticlePhoto === "") ? $this->noPhoto : "https://toko.ua/uploads/images/catalogue/" . $mainArticlePhoto;

            $link = $this->getSiteLink() . $this->products_link . "/" . mb_strtolower($this->getArticleSearch($artId)) . "-" . $this->getBrandLink($brandId) . "-$artId/";
    ...
    public function addComment($art_id, $user_id, $comment)
    {
        $this->addArtReview($user_id, $art_id, $comment);
        return $this->replaceLang($this->getArtReviewsList($art_id));
    }

    public function dropComment($id, $art_id)
    {
        $db = DbSingleton::getTokoDb();
        $db->query("DELETE FROM `T2_ARTICLES_COMMENTS` WHERE `ID` = $id;");
        return $this->replaceLang($this->getArtReviewsList($art_id));
    }

    public function getArtReviews($art_id): string
    {
        $user_id = $this->getUser();
        return $this->getArtReviewsForm($art_id, $user_id) . $this->getArtReviewsList($art_id);
    }
    ...
    public function getArtReviewsForm($art_id, $user_id)
    {
        ...
        if ($user_id > 0 && $art_id > 0) {
            ...
            if ($n > 0) {
                $form = $this->replaceLang($this->getHtmlForm("article/review"));
    ...
    public function addArtReview($user_id, $art_id, $comment): bool
    {
        $db = DbSingleton::getTokoDb();
        $db->query("INSERT INTO `T2_ARTICLES_COMMENTS` (`USER_ID`, `ART_ID`, `COMMENT`, `DATA`, `STATUS`) VALUES ($user_id, $art_id, '$comment', NOW(), 1);");
        return true;
    }
    ...
    {
        $db = DbSingleton::getTokoDb();
        $r = $db->query("SELECT * FROM `T2_SITE_CONFIGS` WHERE `STATUS` = 1 LIMIT 1;");
        $class = $db->result($r, 0, "STYLES");
        $text = $db->result($r, 0, "TEXT");
        $form = getHtmlForm("last_dance");
        return str_replace(array("{class}", "{text}"), array($class, $this->replaceLang($text)), $form);
    }
    ...
    public function getArtReviewsList($art_id)
    {
        ...
        if ($art_id > 0) {
            ...
            $n = $db->num_rows($r);
            if ($n > 0) {
                $form = $this->replaceLang($this->getHtmlForm("article/reviews"));
                $list = "";
                $client = new ClientClass();
                for ($i = 1; $i <= $n; $i++) {
                    ...
                    [$user_name] = $client->getClientUserData($user_id);

                    $comment_delete = "";
                    if ($this->getUser() === $user_id) {
                        $comment_delete = "<button class='btn btn-danger' onClick=\"dropComment('$id','$art_id');\">{delete_cap}</button>";
                    }

                    $list = str_replace(array("{review_username}", "{review_comment}", "{review_date}", "{comment_delete}"), array($user_name, $text, date("d-m-Y", strtotime($date)), $comment_delete), $list);
                }
