...
class ProfileClass {
    ...
    function getSpecialOffers() { $db=DbSingleton::getDbm();
        ...
        $r=$db->query("SELECT * FROM `A_CLIENTS_USERS` WHERE `id`='$user' LIMIT 1;");
    ...
    function getNewsInfo() { $db=DbSingleton::getDbm(); $dbt=DbSingleton::getTokoDb();
        $language=new LangClass; $prefix=$language->getLangPrefix();
        $user=$this->getUser(); $lang=$language->getLanguage(); if ($lang!=1) $lang=5;
        $r=$db->query("SELECT * FROM `A_CLIENTS_USERS` WHERE `id`='$user' LIMIT 1;");
        ...
        if ($user>0 && $n>0) $counter="<span class=\"span-red\">($n)</span>"; else $counter="";
        $info="
        <li>
    ...
    function getProfileInfoMobile() {
        $user=$this->getUser();
        $user==0 ? $info="<a href=\"$this->page_signin\">{authorization}</a>" : $info="<a href=\"$this->page_profile\">{profile}</a>";
    ...
    function showProfileOrders() { $db=DbSingleton::getDbm();
        ...
        $where="`client_user_id`='$user'"; $k=$summ=0; $list="";
        ...
        for ($ii=1;$ii<=$nn;$ii++) {
            ...
            if ($summ>0 && $id!="") {
                $list.="<tr class=\"$bg_bug pointer\" onclick='closeOrderUpdate(\"$id\",\"\")'>
                    <td>$prefix-$id</td>
                    <td>$name</td>
                    <td>$date</td>
                    <td>$city_name</td>
    ...
    function showProfileOrdersArts($dp_check,$order_check) { $db=DbSingleton::getDbm();
            ...
            for ($jj=0; $jj<count($dp_array); $jj++) { $nedp=false;
                ...
                if ($ndp>0) {
                    ...
                    for ($j=1; $j<=$nstr; $j++) {
                        ...
                        if ($this->checkOrderUser($order_id,$user)>0) {
                            $list.="<tr class=\"$bg_bug\">
                                <td>$prefix-$dp_id</td>
                                <td>$article_nr_displ</td>
                                <td>$brand_name</td>
        ...
        if ($dp_check=="") {
            ...
            $r = $db->query("SELECT * FROM `orders_new` WHERE `client_user_id`='$user' AND `dp_id`=0 AND `status`=1 $where_order;"); $n=$db->num_rows($r);
    ...
    function showProfileCheck($data_from="", $data_to="") { $db = DbSingleton::getDbm();
        ...
        $list=$form=""; $client_id=$this->getClient();
    ...
    function setPriceList() { $db=DbSingleton::getDbm();
        $user=$this->getUser(); $date=date("Y-m-d H:i:s"); $date_format=date("Y-m-d_H-i-s");
        $filename="TOKO_GROUP_price-list_".$user."_".$date_format.".csv";
        $r=$db->query("SELECT `status` FROM `cron_task_prices` WHERE `user_id`='$user' AND `status`=1;"); $n=$db->num_rows($r);
        if ($n>0) return "forming..."; else {
            $db->query("INSERT INTO `cron_task_prices` (`user_id`,`filename`,`date`,`status`) VALUES ('$user','$filename','$date',1);");
    ...
    function getPriceList() {
        ...
        $user=$this->getUser(); $date=date("Y-m-d_H-i-s"); $csv="";
        $filedir=$user."/".$user."_price-list_".$date.".csv";
        $filename=$user."_price-list_".$date.".csv";
	...
	function showPriceList() { $db=DbSingleton::getDbm();
		$form=$this->getHtmlForm("profile/profile_price_list"); $user=$this->getUser();
		$disable="disabled"; $visible="style=\"display:none;\""; $history_form="";
		$r=$db->query("SELECT * FROM `cron_task_prices` WHERE `user_id`='$user' and `status`=1;"); $n=$db->num_rows($r);
