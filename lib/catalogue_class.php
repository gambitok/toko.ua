...
class CatalogueClass
{
    ...
    function showCatalogueList($article_nr_search, $brand_nr_search, $status_brand=0) { $db = DbSingleton::getTokoDb();
        ...
        GROUP BY t2c.`ART_ID` ORDER BY t2n.NAME ASC;"); $n = $db->num_rows($r);
        $art_id_str = "";
        for ($i=1; $i<=$n; $i++) {
    ...
    function techModelsList($typ_id, $str_id) { $db = DbSingleton::getTokoDb();
        ...
        $r = $db->query("SELECT `ART_ID` FROM `T2_TREE` WHERE `ART_ID` IN ($t2_link_arts) AND `STR_ID`=$str_id;"); $n = $db->num_rows($r);
        $t2_tree_arts = [];
        for ($i=1; $i<=$n; $i++) {
            $art_id = $db->result($r,$i-1,"ART_ID");
            $t2_tree_arts[] = $art_id;
    ...
    function getSearchFilters($search_filters, $filters, $cur, $current_value, $type_filter, $template_id) {
        ...
        switch ($type_filter) {
            case 1:  {$jsFilter = "catalogueFilter();"; $jsFilterNull = "catalogueFilterNull();"; $jsTextFilter = "search"; $jsFilterClear = "location.reload(true);"; break;}
    ...
    function countBrandItems($article_nr_search, $brand_id) { $db=DbSingleton::getTokoDb();
        ...
        $r = $db->query("SELECT t2c.ART_ID FROM `T2_CROSS` t2c
            LEFT OUTER JOIN `T2_BRANDS` t2b ON t2b.BRAND_ID=t2c.BRAND_ID
            LEFT OUTER JOIN `T2_NAMES` t2n ON t2n.ART_ID=t2c.ART_ID
        WHERE t2c.SEARCH_NUMBER='$article_nr_search' AND t2c.BRAND_ID=$brand_id AND (CASE WHEN t2n.LANG_ID!=NULL THEN t2n.LANG_ID=16 ELSE TRUE END);"); $n = $db->num_rows($r);
        for ($i=1; $i<=$n; $i++) {
            $art_id = $db->result($r,$i-1,"ART_ID");
            $art_id_str.="'$art_id'"; if ($i<$n){$art_id_str.=",";}
        }
    ...
    function shortSearchList($art_id_search) { $db = DbSingleton::getTokoDb();{}
        ...
        for ($i=1; $i<=$n; $i++) {
            $art_id = $db->result($r, $i-1, "ART_ID");
            if ($art_id_search!=$art_id) $where_art_id_str.="'$art_id',";
        }
    ...
    function searchListFilter($where_art_id_str, $article_nr_search, $brand_filter, $text_filter, $cur, $price_min, $price_max, $del_min, $del_max, $brand_nr_search, $order_value, $type_filter=1) { $db = DbSingleton::getTokoDb();
        ...
        $view = $client->getProductView();
        $client_id = $this->getClient(); $tpoint_id = $client->getTpoint();
    ...
    function printSearchList($id, $art_id, $article_name, $brand_id, $brand_name, $text, $delivery_info, $stock, $price, $article_nr_search, $ll, $class, $hide, $border, $none, $brand_nr_search, $suppl_id, $return_days, $delivery_days, $delivery_short_info, $storage_id, $status, $view) {
        ...
        $cur = $kours->getCurrentKours(); $kours_cap = $kours->getKoursSymbol($cur);
    ...
    function getTpointDeliveryInfo($tpoint_id, $storage_id) { $db = DbSingleton::getTokoDb();
        ...
        if ($n==1) {
            ...
            $date_del = date("d.m", strtotime(" + ".$delivery_days." days"));
            if ($delivery_days==0)
                $today = "<i>{today_cap}</i>"; elseif ($delivery_days==1) $today = "<i>{tomorrow_cap}</i>";
            else
                $today = "<i>$date_del ($week_day_short)</i>";
            $info = "$today<br>$time_from_del - $time_to_del";
    ...
    function sortByMinStock($mas) {
        ...
        foreach ($mas as $mas_key=>$mas_val) {
            foreach ($mas_val as $key=>$val) {
                if ($min_key!=0) {
                    if ($mas[$pred_key][0]["price"]>$mas[$pred_key][$min_key]["price"] && $mas[$pred_key][0]["dd"]>$mas[$pred_key][$min_key]["dd"]) $null_key = 0; else $null_key = 1;
    ...
    function searchListTest($where_art_id_str, $type_filter=1, $view=0) { $db = DbSingleton::getTokoDb();
        ...
        if ($where_art_id_str!="") {
            ...
            FROM `T2_ARTICLES` t2a
                ...
                LEFT OUTER JOIN `T2_SUPPL_IMPORT` t2si ON (t2si.art_id=t2a.ART_ID AND t2si.status=1)
            WHERE t2a.ART_ID IN ($where_art_id_str) AND t2b.`VISIBLE`='1' AND (CASE WHEN t2n.LANG_ID!=NULL THEN t2n.LANG_ID=16 ELSE TRUE END)
            GROUP BY t2a.ART_ID, t2si.client_storage_id"); $n = $db->num_rows($r);
            $list = $this->drawHeaderSearchList($type_filter, $view);
