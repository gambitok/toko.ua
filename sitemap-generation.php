...
for ($j=1;$j<=$n2;$j++) {
    ...
    $r1=$db->query("SELECT * FROM `T_manufacturers` WHERE `ACTIVE`=1 ORDER BY `MFA_ID`;"); $n1=$db->num_rows($r1);
    for ($l=1;$l<=$n1;$l++) {
        ...
        $r=$db->query("SELECT * FROM `T_models` WHERE `MOD_MFA_ID`='$mfa_id' GROUP BY `Model` ORDER BY `Model`;"); $n=$db->num_rows($r);
        for ($i=1;$i<=$n;$i++) {
